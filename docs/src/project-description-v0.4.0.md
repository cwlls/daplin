...
Daplin solves the trust problem the same way humans have always solved it — through relationships, through vouching, through the web of people who already know each other. It wraps that human reality in an open protocol that is portable, federated, and requires no one's permission to use. It is infrastructure, not an application. Messaging clients, email systems, and other communication tools build on top of Daplin rather than replicating it.
...
- **Sync Key** — derived deterministically from the master key; encrypts the user's own profile plaintext for multi-device sync via IPFS; identical across all of the user's authorised devices

### Identity Sharing — Deep Links and Seal Endpoints

Daplin does not use human-readable handles. Handles introduce spoofing risk by conditioning users to trust a text string rather than a cryptographic identity. The DID is the canonical identity. Display names are convenience labels on cards, not identifiers.
...
Deep links are shared as QR codes, clickable links, or NFC payloads. They are never typed by users.

Every instance also serves a seal endpoint — a known URL pattern that generates the user's visual seal from their DID hash, similar to how Gravatar provides avatar images:
...
- **Base seal** — deterministic geometry generated from the DID hash alone. Generatable by any Daplin client without contacting any server. Always identical for a given DID.
- **Personalised seal** — base seal with the user's chosen palette and border applied. Served by the instance. Falls back to base seal if instance is offline.

The instance serves the personalised seal but cannot falsify the locked geometry — any tampering is detectable by anyone who runs the base seal generation algorithm against the DID.
...
### IPFS as a Separate Concern

The Daplin instance software communicates with IPFS via a defined storage interface. IPFS is not bundled into the instance software — each component does one job. Operators choose their own IPFS backend:

- **Docker Compose stack (recommended)** — the reference deployment pairs the Daplin instance with an IPFS node (ipfs/kubo) in a single compose file. One command to run a full self-hosted node.
- **External IPFS node** — for operators who already run IPFS infrastructure or want full control.
- **Pinning service** (e.g. Pinata) — for managed hosting scenarios with no node maintenance.

Operators substituting their own backend simply omit the IPFS service from the compose stack and configure the storage interface accordingly.

### What Lives Where

| Layer | Contents |
|-------|----------|
| **IPFS** (public, permanent) | Cards, attestations, key rotation records, migration declarations, recovery declarations, encrypted contact blobs, encrypted sync blob |
| **Device Secure Enclave** | Master key, sync key, device subkeys, pairwise keys (one per relationship) |
| **App Local Storage** (encrypted) | Profile plaintext (working copy), card CIDs (contact list), received capability keys, offline activity queue |
| **Instance** (transient only) | Event queue, DID-to-CID resolution index, contact blob CID pointers, sync blob CID pointer |
| **Never stored anywhere** | Other users' contact details (always fetched fresh from IPFS) |

Pairwise DIDs must never be published to IPFS — even encrypted, their presence would constitute an observable graph edge. The instance never receives plaintext of any kind.

### Multi-Device Profile Sync

Alice's profile plaintext is available and editable across all her devices via IPFS, without the instance ever seeing it:

- On save, the app encrypts the updated plaintext with the sync key and publishes the result to IPFS as a sync blob
- The instance stores only the sync blob CID pointer
- Any of Alice's devices can fetch the sync blob CID from the instance, retrieve the blob from IPFS, and decrypt it with the sync key
- The sync key is derived from the master key and is therefore identical on every authorised device

The IPFS round-trip only occurs when opening the profile editor, which is infrequent. The write path happens in the background after the UI confirms the save.
...
|---------|--------|-------|
| v1.0 | did:key + did:peer | Global identity + pairwise channels. No infrastructure required. |
